...
if __name__ == "__main__":
    ...
    SYMBOLS = ["ADAUSDT", "BNBBTC", "BNBUSDT", "ETHUSDT"]
    ...
    if features_conf["Time_Features"]:
        ...
        if features_conf["Time_Fourier_Features"]:
            data = fe.build_fourier_time_features(
                data,
                time_levels=["month", "day", "hour", "minute"],
    ...
    if features_conf["Apply_Standard_Scaler"]:
        ...
        for col in cols:
            data.loc[:, f"{col}_standard"] = data.groupby("Ticker")[col].transform(
                lambda x: fe.apply_standard_scaler(x)
            )
    ...
    sub_experiment_type = "conf_{}_Tickers_{}_Stategy_{}".format(
        feature_id, data["Ticker"].nunique(), strategy
    )

    output_location = os.path.join(
        output_folder_db, ("Feature_Engineering_{}.parquet".format(sub_experiment_type))
    )
