...
def main():
    ...
    denoising_log_dir = os.path.join(
    ...
    LOG_FILENAME = os.path.join(denoising_log_dir, f"{timestamp}_Denoising_return.log")
    ...
    for ticker in SYMBOLS:
        ...
        local_timestamps = df_ticker["timestamp_local"].iloc[-subset_wavelet_transform:]
        ...
        max_lag_pacf = get_significant_max_lag_pacf(
            pacf_values=pacf_values, confint=confint, lags_pacf=lags_pacf
        )
    ...
    df_prices["timestamp_return_ref"] = df_prices["timestamp_local"].apply(
        lambda x: x + timedelta(minutes=minute_sampling)
    )

    df_prices = df_prices.filter(
        [
            "timestamp_local",
            "timestamp_return_ref",
            "price_returns",
            "denoised_price_returns",
            "ticker",
            "pacf_lag",
        ]
    ).copy()
    ...
    df_prices.to_parquet(
        os.path.join(output_data_path, "Tensor_Portfolio_denoised.parquet")
    )
