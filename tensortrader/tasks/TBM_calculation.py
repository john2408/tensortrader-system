...
if __name__ == "__main__":
    ...
    for symbol in SYMBOLS:
        for year in years_filter:
            ...
            file_name = os.path.join(
                input_folder_db, f"{symbol}", f"{year}_{symbol}.parquet"
            )
    ...
    data = (
        data.groupby(["Ticker"], group_keys=False).apply(reindex_by_date).reset_index()
    )
    ...
    if True:
        ...
        delta_vertical_b = pd.Timedelta(
            minutes=v_barrier_minutes
        )  # Vertical barrier length
        output_folder_db = os.path.join(
            path_prefix, "Data/Tensor_Invest_Fund/data/Cryptos/TBM"
        )

        current_date = datetime.now().strftime("%Y-%m-%d-%H-%M")
        LOG_FILENAME = os.path.join(
            os.getcwd(), "logs", f"{current_date}_TBM_labels.log"
        )
        logging.basicConfig(
        ...
        for ptsl in PTSL:
            ...
            for ticker in SYMBOLS:
                ...
                try:
                    TBM_parallel = TripleBarrierMethod(
                        df_sub,
                        ticker,
                        ptsl,
                        delta_vertical_b,
