...
def main():
    ...
    LOG_FILENAME = os.path.join(logs_folder, f"{timestamp}_Training_ML_return.log")
    ...
    try:
        ...
        path_loc = os.path.join(
            input_data_path_denoised_return, db_name_denoised_return
        )
    ...
    for ticker in SYMBOLS:
        ...
        forecast_train = ticker_tcn_model.scaler.inverse_transform(
            ticker_tcn_model.model.predict(X_train)
        ).reshape(1, -1)[0]
        forecast_test = ticker_tcn_model.scaler.inverse_transform(
            ticker_tcn_model.model.predict(X_test)
        ).reshape(1, -1)[0]

        original_train = ticker_tcn_model.scaler.inverse_transform(Y_train).reshape(
            1, -1
        )[0]
        original_test = ticker_tcn_model.scaler.inverse_transform(Y_test).reshape(
            1, -1
        )[0]
        ...
        print(
            ...
            np.round(pearson_coff, 3),
            "for one step ahead forecast",
        )

        df_metric = pd.DataFrame({"ticker": [ticker], "pearson_coff": [pearson_coff]})
