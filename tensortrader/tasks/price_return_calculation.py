...
def main():
    ...
    LOG_FILENAME = os.path.join(price_return_dir, f"{timestamp}_Price_return.log")
    ...
    logger.info(
        f"""Running Price Return for Symbols: {SYMBOLS}
    ...
    data = Loader.load(n_days=n_days, symbols=SYMBOLS)

    info = "Loading data from {} until {}, years filter are {}".format(
        Loader.initial_date_load, Loader.initial_date_load, Loader.years_filter
    )
    print(info)
    logger.info(info)
    ...
    data["timestamp_local"] = data["timestamp"].apply(
        lambda x: utc_to_local(x, local_tz)
    )
