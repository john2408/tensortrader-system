...
def main(symbol):
    ...
    backtesting_folder = os.path.join(
    ...
    LOG_FILENAME = os.path.join(
        backtesting_folder, f"{current_date}_Backtester_{symbol}.log"
    )
    ...
    logger.info(
        f"""Running Backtesting for Symbols: {symbols}
    ...
    if label_mode == "return":
        ...
        target_col_name = "{}_target_return_{}m".format(
            variable, int(resampling[:-3]) * return_lag
        )
    ...
    train_period_length = (
        None
        if CONF.get("train_period_length") == "None"
    ...
    data = Loader.load(n_days=n_days, symbols=symbols, years_filter=years_filter)
    ...
    if label_mode == "TBM":
        ...
        for ticker in symbols:
            ...
            TBM_labels = TripleBarrierMethod(
                df,
                ticker,
                ptsl,
                delta_vertical_b,
    ...
    backtesting_df_TBM = Signals_Backtest.vectorize_backtesting(
        ds=data.copy(),
    ...
    feature = FeatureEngineer(feature_id=feature_id, conf_path=conf_path)
    ...
    X_train, X_test, y_train, y_test = feature.train_test_split_multiple_ts(
        data, predictors_list, train_size, target_variable
    )
    ...
    if calculate_feat_importance:
        ...
        feat_importance = feature.feature_selection(
    ...
    XGB_trainer.store_model(model_name=model_name, storage_folder=backtesting_folder)
    ...
    if target_type == "regression":
        ...
        df_signals = pd.DataFrame(y_pred.rename(R_Signals.target_col_name)).join(
            data.set_index("Date").filter(["threshold"])
        )

        df_signals["label"] = R_Signals.calculate_signals(df_signals, long_short)

        backtesting_df_ML = Signals_Backtest.vectorize_backtesting(
            ds=data.copy(),
            modus="ML_performance",
