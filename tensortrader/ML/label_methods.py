...
class ReturnSignal:
  ...
  """
    ...
    """
        ...
        signals = np.where(
            df[self.target_col_name] < df["threshold"] * short, -1, signals
        )
    ...
    """
        ...
        if return_type == "simple":
            returns = (
                data.set_index([timestamp_col])[variable]
                ...
                .shift(
                    -return_lag
                )  # Shift Target variable to current candle --> To allow ML Regression Training
...
class TripleBarrierMethod:
  ...
  """
    ...
    """
        ...
        return pd.DataFrame(
            {"thresh_uppr": thresh_uppr, "thresh_lwr": thresh_lwr}, index=events.index
        )
    ...
    """
      ...
      """
        ...
        parallel_outputs = Parallel(
            n_jobs=self.n_jobs, max_nbytes=self.max_nbytes, verbose=self.verbose
        )(
            delayed(parallel_touch)(Close, events, loc, t1)
            for loc, t1 in events["t1"].iteritems()
        )
    ...
    """
      ...
      """
        ...
        parallel_outputs = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(
            delayed(parellel_labels)(touches, loc, t)
            for loc, t in first_touch.iteritems()
        )
