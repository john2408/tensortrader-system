...
class ETL:
    def __init__(
        self, load_size_days, end_timestamp, start_timestamp=None, total_days=None
    ):
        """ETL Metaclass
        ...
        if self.total_days is not None:
            ...
            for lot in range(1, int(self.total_days / self.load_size_days)):
                self._import_timestamps.append(
                    (
                        self.end_timestamp - timedelta(days=lot * self.load_size_days),
                        self.end_timestamp
            ...
            while end_time < self.end_timestamp:
                ...
                start_time = self.start_timestamp + timedelta(
                    days=(lot - 1) * self.load_size_days
                )
                end_time = self.start_timestamp + timedelta(
                    days=lot * self.load_size_days
                )

                if end_time > self.end_timestamp:
                    end_time = self.end_timestamp

                self._import_timestamps.append((start_time, end_time))
...
class ETL_Binance(ETL):
    ...
    def __init__(
        ...
        super().__init__(load_size_days, end_timestamp, start_timestamp, total_days)
    ...
    def update_data(self, interval, limit=1000, verbose=0):
        ...
        for ticker in self._tickers:
            ...
            if not df_new.empty:
                ...
                self.store_update_as_parquet(
                    df_new, ticker, year=max(df_new["Year"].unique())
                )

                if verbose > 0:
                    print(
                        "Sucessfully data update for",
                        ticker,
                        "New Update at",
                        df_new["Date"].max(),
                    )
    ...
    def retrieve_historical_data(self, ticker, interval, limit=1000):
        ...
        for start_str, end_str in self._import_timestamps:
            ...
            try:
                bars = self._client.get_historical_klines(
                    symbol=ticker,
                    interval=interval,
    ...
    def select_latest_ticker_file(self, ticker):
        ...
        lastest_file = os.path.join(
            self.storage_folder, ticker, f"{latest_update_year}_{ticker}.parquet"
        )
...
class DataLoader:
    ...
    def load(self, n_days, symbols) -> pd.DataFrame:
        ...
        self.initial_date_load = (
            self.current_timestamp - dateutil.relativedelta.relativedelta(days=n_days)
        )
        ...
        for symbol in symbols:
            for year in self.years_filter:
                ...
                file_name = os.path.join(
                    self.input_folder_db, f"{symbol}", f"{year}_{symbol}.parquet"
                )
